...
class PersonCtrl {
    ...
    public function processRequest()
    {
        switch ($this->requestMethod) {
            case 'GET':
                ...
                break;
            case 'POST':
                echo '<< '. $this->requestMethod .'>><<'.$this->userId.'>>';
                if ($this->userId){
    ...
    private function createUserFromRequest()
    {
        ...
        $insertados=$this->personGateway->insert($_POST);
        $response['status_code_header'] = 'HTTP/1.1 201 Created';

        $this->resp->ok='true';
        $this->resp->message='un registro insertado correctamente';
